...
public class DoubleUIField extends NetworkUIPort<Double> implements TitleLabelProvider {

    private final FormattedLabel title;
    private final EditBox field;

    public DoubleUIField(Consumer<Double> write, Supplier<Double> read, Component titleText) {
        super(write, read);
    ...
    public DoubleUIField(Consumer<Double> write, Supplier<Double> read, LabelProvider titleText) {
        super(write, read);
        title = titleText.toDescriptiveLabel();
        field = new EditBox(Minecraft.getInstance().font, 0, 0, 60, 10, Component.literal(""));
    }

    public static DoubleUIField of(Consumer<String> write, Supplier<String> read, Component titleText) {
        return new DoubleUIField(
                d -> write.accept("" + d),
    ...
    public static DoubleUIField of(Consumer<String> write, Supplier<String> read, LabelProvider titleText) {
        return new DoubleUIField(
                d -> write.accept("" + d),
                () -> ParseUtils.tryParseDouble(read.get()),
                titleText
